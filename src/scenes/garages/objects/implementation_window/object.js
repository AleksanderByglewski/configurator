...
class WindowObject extends genericObject {
    constructor() {
        super();
    }
    create(attributes = {}) {
        ...
        let depth = (attributes && attributes.window_depth) ? parseFloat(attributes.window_depth) : 0;
        let height = (attributes && attributes.door_height) ? parseFloat(attributes.door_height) : 0;
...
class LineObject extends genericObject {
    constructor() {
        super();
    }
    create(attributes = {}) {
        ...
        let sheet_depth = (attributes && attributes.sheet_depth) ? parseFloat(attributes.sheet_depth) : 10.25;
        let stilt_width = (attributes && attributes.stilt_width) ? parseFloat(attributes.stilt_width) : 0;
        let stilt_depth = (attributes && attributes.stilt_depth) ? parseFloat(attributes.stilt_depth) : 0;
        ...
        geometry = new THREE.BoxGeometry(
            stilt_width,
            height,
            stilt_depth, // Assuming depth is always 1, adjust as needed
       
        );
        
        // material = select_texture({ width: 1.2 * width, height: height, color: color, material_type: material_type })
        //  material = new THREE.MeshStandardMaterial({
        //     color: 0xcecece , // This is black color
        //     metalness: 0.91, // Full metallic effect
        //     roughness: 0.03 // Smooth surface for shiny metal effect
        // });

        material = select_texture({ width: 10 * width, height: 10*height, color: color, material_type: material_type })

        // const mesh = new THREE.Mesh(geometry, material);

        let mesh
        const group = new THREE.Group();
        const gap = 0.1; // Gap between boxes
...
class CentralLineObject extends genericObject {
    ...
    create(attributes = {}) {
        ...
        // material = select_texture({ width: 1.2 * width, height: height, color: color, material_type: material_type })
         material = new THREE.MeshStandardMaterial({
            color: 0x333335, // This is black color
...
export { SphereObject, WindowObject, DoorHandleObject, CentralLineObject, LineObject }
