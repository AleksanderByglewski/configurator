...
//        <img src="${button.display_image}" alt="${button.display_value}" style="aspect-ratio: 1 / 1; object-fit: cover;">
    ...
    static createInputField(args) {
        ...
        function createDimensionChangeHandler(dimensionType) {
            return function(e) {
              const value = e.target.value; // Get the input value from the event target
              const stateChangeObject = {};
              stateChangeObject[dimensionType] = value; // Dynamically create the property based on dimensionType
              this.notifyMediator('stateChange', stateChangeObject);
